...
int salva_qtd_jogadas(int qtdJogadas, int ganhador)
{
    ...
    FILE *f = fopen("leaderboards.txt", "a");
    ...
    if (ganhador == 1)
    {
        fprintf(f, "Data: %s \t Ganhador: Player com %d jogadas.\n", ctime(&now), qtdJogadas);
    }
    else
    {
        fprintf(f, "Data: %s \t Ganhador: Bot com %d\n jogadas.", ctime(&now), qtdJogadas);
    }
    fclose(f);
}
...
int main()
{
    ...
    printf("Resultado salvo no arquivo de leaderboards.txt\n");
    printf("Deseja jogar novamente? Sim (s) ou Nao (n)\n");
    while (1)
    {
        scanf("%c", &op);
        switch (op)
